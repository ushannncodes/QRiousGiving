#!/usr/bin/env python3
# cam-final3.py  — polarity + robust ASCII preview edition
# - Queue-backed serial sender (_queue_send)
# - Motion delta + heartbeat JSON
# - Polarity switch FLIP_BLACK_IS (0 => bit=1=white; 1 => bit=1=black)
# - Boot frame respects polarity
# - ASCII preview that still works under kiosk (FORCE_ASCII, ASCII_LOG)
# - Cadenced sender + single-line HUD (can disable with HB_DEBUG=0)
...
# HUD debug (single status line)
HB_DEBUG           = bool(int(os.getenv("HB_DEBUG", "0")))          # default OFF to not overwrite ASCII

# ASCII robustness (NEW)
FORCE_ASCII        = bool(int(os.getenv("FORCE_ASCII", "0")))       # print even if not a TTY
ASCII_LOG          = os.getenv("ASCII_LOG", "")                     # path to write ASCII grid (for tail -f)
...
def _render_ascii_grid(bw: np.ndarray):
    """Robust ASCII grid: prints to stdout if TTY or FORCE_ASCII; also writes to ASCII_LOG path if set."""
    grid = "\n".join("".join('#' if bw[y, x] else '.' for x in range(28)) for y in range(28))
    text = "[Preview] '#'=black, '.'=white\n" + grid + "\n(Ctrl+C to quit)\n"
    if ASCII_LOG:
        try:
            with open(ASCII_LOG, "w") as f:
                f.write(text)
        except Exception as e:
            print(f"[ASCII] write error: {e}")
    if sys.stdout.isatty() or FORCE_ASCII:
        try:
            os.system("clear")
        except Exception:
            pass
...
def main():
    ...
    # One-time boot frame (polarity-aware)
    try:
        boot = np.zeros((28,28), dtype=np.uint8)   # 0=white in our bw convention
        if BLACK_IS == 0:
            boot = 1 - boot
        _queue_send(boot)
        time.sleep(0.05)
        print("[BOOT] Sent full-white (polarity-aware) frame.")
    except Exception as e:
    ...
    while running_flag["run"]:
        ...
        # To 28x28
        if USE_GUI:
            bw, debug_panel = to_flipdot_matrix(
                bgr, ROI_BOX=tuple(ROI_BOX), mirror=MIRROR,
                ...
                return_debug=True, overlay_roi=False
            )
            cv2.imshow("cam|mask|28x28", debug_panel)
            cv2.waitKey(1)
        else:
