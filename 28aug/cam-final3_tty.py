#!/usr/bin/env python3
# cam-final3_tty.py — ASCII preview forced to the terminal device (/dev/tty) if stdout is not visible.
# Same as your cam-final3.py, but _render_ascii_grid writes to /dev/tty fallback.
...
USE_GUI            = bool(int(os.getenv("USE_GUI", "0")))  # default OFF
USE_ASCII          = bool(int(os.getenv("USE_ASCII", "1"))) # default ON
...
BLACK_IS           = int(os.getenv("FLIP_BLACK_IS", "0"))  # your default
...
def _write_tty(text: str):
    ...
    if ok:
        return
    # Fallback: write to /dev/tty directly
    try:
        ...
        ok = False
    # Also mirror to file if requested
    if ASCII_LOG:
        try:
...
def main():
    ...
    while running_flag["run"]:
        ...
        due_min = (now - last_send) >= 0.15
        due_max = (now - last_send) >= 1.0
        ...
        if HB_DEBUG and (now - dbg_last) >= 0.25:
            if is_active:
