#!/usr/bin/env python3
# cam-final2_ascii_flip.py
# - Camera: Picamera2 (like your cam-final2.py), so no V4L2 timeout.
# - ASCII preview printed reliably to stdout.
# - Flip-dot sending via queue + pack_flipbytes (no undefined helpers).
# - Polarity switch: BLACK_IS (env FLIP_BLACK_IS, default 0 for your board).
# - Boot-white at start (polarity-aware).
# - Mediapipe pose+hands pipeline, zoom, heartbeat, motion delta kept.
#
# Run example:
#   USE_GUI=0 USE_ASCII=1 FLIP_BLACK_IS=0 SEND_LOG=1 \
#   python3 ~/Desktop/cam-final2_ascii_flip.py
...
HEAD_OVAL_SCALE    = 1.10
NECK_RATIO         = 0.42
NECK_LEN_RATIO     = 0.30
SHOULDER_SPREAD    = 1.10
SHOULDER_DEPTH     = 0.28
...
# Polarity (your board needs 0)
BLACK_IS           = int(os.getenv("FLIP_BLACK_IS", "1"))  

# Misc
HB_DEBUG           = bool(int(os.getenv("HB_DEBUG","0")))
...
def main():
    ...
    # Boot white (polarity-aware)
    try:
        boot = np.zeros((28,28), dtype=np.uint8)
        if BLACK_IS == 0: boot = 1 - boot
        _queue_send(boot); time.sleep(0.05)
        print("[BOOT] Sent full-white (polarity-aware) frame.")
    except Exception as e:
        print(f"[BOOT] send failed: {e}")

    last_send = 0.0; last_frame=None; ema_delta=0.0
    ...
    while running["run"]:
        ...
        now = time.time()
        if (now - getattr(main, "_hb_last", 0.0)) >= HEARTBEAT_EVERY:
            try:
                state = {"ts": now, "active_pixels": active_pixels, "ema": float(active_pixels),
                         "delta": int(delta_pixels), "delta_ema": float(ema_delta),
                         "active": bool(active_pixels >= ACTIVE_PIXELS_MIN)}
