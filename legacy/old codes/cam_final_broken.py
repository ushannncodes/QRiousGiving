...
import os, cv2, time, time, signal, numpy as np, serial, threading, queue, json
...
def serial_sender(port, baud, panel_addrs, pkt_queue: queue.Queue, running_flag):
    ...
    try:
        ...
        print(f"[SER] Closed at {time.time():.3f}s")
...
def main():
    ...
    if BOOT_MODE in ("white","black"):
        try:
            ...
            global paused_last
            if paused_last:
                print(f"[PAUSE] resumed t={time.time():.3f}")
                paused_last = False
    ...
    while running["run"]:
        ...
        #     os.system("clear")
        #     print(f"[{time.time():.3f}] " + f{cam[cam.find("f?")+1:cam.find(")")]} )
        ...
        if (due_min and big_change) or due_max:
            # If pause flag exists, skip sending but keep cadence fresh
            if os.path.exists(PAUSE_FLAG):
                global paused_last
                if not paused_last:
                    print(f"[PAUSE] flipdot writes paused t={time.time():.3f}")
                    paused_last = True
                last_send = now
                last_frame = out.copy()
                continue
