# anim_sprout_breeze_locked.py
# Phases:
#   1) Stem grows (mask-driven)
#   2) Leaves bloom out slowly (distance field)
#   3) Breeze sway (leaves only) with "lock-to-stem" connectors to avoid detaching
#
# Notes:
# - 0 = black(ON), 1 = white(OFF) for flipdots
# - Keeps global +1px RIGHT shift
# - Uses your exact timing & breeze settings
...
# ---- Plant (Phases 1–2) ----
DURATION    = float(os.getenv("DURATION", "4.5"))   # core plant timing unchanged
...
BREEZE_HZ      = float(os.getenv("BREEZE_HZ", "1.4"))   # cycles per second
BREEZE_AMPL    = int(os.getenv("BREEZE_AMPL", "1"))     # px; 1 looks best on flipdots
BREEZE_PHASEDY = float(os.getenv("BREEZE_PHASEDY", "0.18"))  # per-row phase offset
BREEZE_DECAY   = float(os.getenv("BREEZE_DECAY", "0.12"))     # 0=no decay
...
# Precompute per-row stem columns for fast connectors
...
def plant_frame_at(i):
    ...
    if t <= PHASE_SPLIT:
        # Stem grows bottom→top with cosine-like ease
...
# ---------------- Breeze (Phase 3) with stem lock ----------------
def breeze_frame_at(k):
    """Leaves sway per-row; if sway opens a gap next to the stem, add a connector dot."""
    ...
    for y in range(H):
        ...
        # ---- lock to stem: add connector black dot if gap opened ----
        # Rule: if there are leaves in this row and the neighbor cell on the sway side
        # of ANY stem column is white, set it black to maintain visual attachment.
        if shift != 0 and leaf_cols.size and STEM_COLS_PER_ROW[y].size:
            sgn = 1 if shift > 0 else -1
            for xs in STEM_COLS_PER_ROW[y]:
                xn = xs + sgn
                if 0 <= xn < W:
                    # only add connector if destination is not stem and is currently white,
                    # and there exists at least one leaf pixel further out in the sway direction
                    if (not STEM_MASK[y, xn]) and (img[y, xn] == 1):
                        # check if there is a leaf somewhere beyond xn in sway direction
                        if sgn > 0:
                            has_leaf_beyond = np.any(dst_row[xn:])
                        else:
                            has_leaf_beyond = np.any(dst_row[:xn+1])
                        if has_leaf_beyond:
                            img[y, xn] = 0  # tiny petiole connector
