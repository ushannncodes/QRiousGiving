...
# Vis / animation
THRESH      = int(os.getenv("THRESH", "250"))  # 0..255; higher makes the hand “grow”
ANIM_STEPS  = int(os.getenv("ANIM_STEPS", "28"))
ANIM_DELAY  = float(os.getenv("ANIM_DELAY", "0.05"))
OUTLINE_PAUSE = float(os.getenv("OUTLINE_PAUSE", "3"))
...
SUPER = int(os.getenv("SUPER", "10"))          # supersample factor (4–10 is typical)
BLUR  = float(os.getenv("BLUR", "1"))       # Gaussian sigma at supersampled size
THRESH = int(os.getenv("THRESH", "200"))      # binarize threshold at 28×28 (0..255)

# ---------- IMAGE → 28×28 MASKS ----------
def load_png_to_mask(path, thresh=THRESH):
    """
    Return 28x28 mask with 1=white (background), 0=black (hand) using
    supersampling + blur + high-quality downscale for smoother edges.
    """
    # load big
    big = Image.open(path).convert("L")
    big = big.resize((28*SUPER, 28*SUPER), Image.LANCZOS)

    # soften jaggies at high-res
    if BLUR > 0:
        big = big.filter(ImageFilter.GaussianBlur(BLUR))

    # downsample with proper antialiasing
    small = big.resize((28, 28), Image.LANCZOS)

    # final threshold at target resolution
    out = []
    for y in range(28):
        row = []
        for x in range(28):
            px = small.getpixel((x, y))
            row.append(1 if px >= thresh else 0)   # 1=white, 0=hand
        out.append(row)
    return out

def _erode(binary, radius=1):
    """
    Erode a binary image (list of lists) using a (2r+2r+1) square struct elem.
    Input/Output: 1=hand, 0=background  (note: we invert below to reuse this)
    """
    H, W = 28, 28
...
def derive_outline(white_bg_mask, thickness=1):
    """
    Outline = hand XOR erode(hand). Thickness is roughly the erosion radius.
    Returns a mask with 0 at outline pixels (so your sender draws them black),
    1 elsewhere (white).
    """
    # convert to hand=1, bg=0
    hand = [[1 - white_bg_mask[y][x] for x in range(28)] for y in range(28)]
    er   = _erode(hand, radius=thickness)     # shrink hand
    # gradient = hand - eroded_hand  (1=outline, 0=not)
    H, W = 28, 28
    outline01 = [[1 if (hand[y][x] == 1 and er[y][x] == 0) else 0 for x in range(W)] for y in range(H)]

    # convert to your convention: 0 at outline pixels, 1 elsewhere
