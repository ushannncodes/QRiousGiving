...
def show_slice(panel_address, t, srl, question):
    transmission = bytearray([
        0x80,  # header
        0x85,  # 56 bytes, refresh
        panel_address,  # address
    ])

    for i in range(t, t + 28):
        col = question[i % len(question)]
        transmission.append(col & 0x7F)  # top 7 bits only

    for i in range(t, t + 28):
        transmission.append(0x00)  # bottom 7 rows all off (empty)
...
#     for i in range(t, t + 28):
#         col = question[i % len(question)] & 0b01111111  # restrict to 7 bits
#         transmission.append(col)
...
with serial.Serial("/dev/ttyS0", 9600, timeout=1) as srl:
    ...
    # 🔍 Manual test to observe panel behavior
    manual_test = bytearray([0x80, 0x83, 0x01])
    manual_test += bytearray([0b00011110] * 28)  # 28 columns, short vertical bars
    manual_test += bytearray([0x8F])
    srl.write(manual_test)

    time.sleep(5)  # Wait 5 seconds to observe result
